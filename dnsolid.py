...
__author__ = 'Ümmühan İlhan'
__version__ = '20180627'
__email__ = 'u.ilhan24@gmail.com'
...
''' % __version__ + FG_RST + ST_RST)
	...
	if args.ssdeep and MODULE_SSDEEP and MODULE_REQUESTS:
		...
		try:
                    req = requests.get(url.get_full_uri(), timeout=REQUEST_TIMEOUT_HTTP, headers={'User-Agent': 'Mozilla/5.0 (dnsolid)'})
	...
	d_cli('Processing %d domain variants ' % len(domains))
	...
	if domains:
		if args.format == 'csv':
			d_csv(generate_csv(domains))
		elif args.format == 'json':
			d_json(generate_json(domains))
		else:
	       		d_cli(generate_cli(domains))
