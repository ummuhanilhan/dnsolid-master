...
The idea is quite straightforward: `dnsolid` takes in your domain name as a
...
If you're on a Mac, you can install dnsolid via
[Homebrew](https://github.com/Homebrew/homebrew) like so:

```
$ brew install dnsolid
```

This is going to install `dnsolid.py` as `dnsolid` only, along with all
...
```
$ docker pullummuhanilhan/dnsolid
$ docker run ummuhanilhan/dnsolid example.com
```
...
```
$ dnsolid.py --registered example.com
```
...
```
$ dnsolid.py --mxcheck example.com
```
...
```
$ dnsolid.py --dictionary dictionaries/english.dict example.com
``` 
...
```
$ dnsolid.py --format csv example.com > out.csv
$ dnsolid.py --format json example.com > out.json
```

In case you want to chain `dnsolid` with other tools and you need only domain
variants without performing any DNS checks, you can use `--format idle`:

```
$ dnsolid.py --format idle example.com | tr '\n' ','
```
...
```
$ dnsolid.py --geoip example.com
```

Of course all of the features offered by `dnsolid` together with brief
...
with `dnsolid` and are comfortable with sharing them, also please send me a
message. Thank you.
